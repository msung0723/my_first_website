...
if (!window.__codexBackdropEventSyncApplied) {
    window.__codexBackdropEventSyncApplied = true;

    let backdropEventSyncTimer = null;

    const queueBackdropEventSync = (forceSeek = false, delay = 0) => {
        if (backdropEventSyncTimer) {
            clearTimeout(backdropEventSyncTimer);
        }

        backdropEventSyncTimer = window.setTimeout(async () => {
            backdropEventSyncTimer = null;

            try {
                await Promise.resolve(applyMusicTrackBackdrop());
                ...
                console.warn("Failed to sync backdrop video from playback events", error);
            }
        }, delay);
    };

    if (musicAudio) {
        musicAudio.addEventListener("play", () => queueBackdropEventSync(true, 40));
        musicAudio.addEventListener("pause", () => queueBackdropEventSync(true, 40));
        musicAudio.addEventListener("seeked", () => queueBackdropEventSync(true, 20));
        musicAudio.addEventListener("loadedmetadata", () => queueBackdropEventSync(true, 20));
    }

    const originalShowPageForBackdropEventSync = showPage;
    showPage = function(pageId, options) {
        originalShowPageForBackdropEventSync(pageId, options);
        queueBackdropEventSync(false, 30);
    };
}
